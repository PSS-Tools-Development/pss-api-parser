...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to the structure JSON file to be used.",
    )
    parser.add_argument(
        "--enums",
        type=str,
    ...
    parser.add_argument(
        ...
        required=False,
        default="python",
        help="Target language to be used (python by default)",
    )
    ...
    with _Timer() as t:
        ...
        if args.cacheable:
            print(
                f"{_Fore.YELLOW} >>>{_Fore.RESET} Cacheable Endpoints: {args.cacheable}"
            )
        ...
        print(
            f'{_Fore.YELLOW} >>>{_Fore.RESET} Overwrite: {"Yes" if args.overwrite else "No"}'
        )
