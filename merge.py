...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        nargs="+",
        required=True,
        help="Path(s) to the flows file(s) to be merged",
    )
    parser.add_argument(
        "--overrides", type=str, required=False, help="Path to the overrides file"
    )
    parser.add_argument(
        "--outfile",
        type=str,
        required=True,
        help="Target file path for the merged flows file",
    )
    parser.add_argument(
        "--uncompressed",
        action="store_true",
    ...
    for file_path in args.in_:
        if not _os.path.isfile(file_path):
            print(
                f"{_Fore.RED}ERROR: Specified flows JSON file does not exist: {file_path}{_Fore.RESET}"
            )
    if error:
    ...
    with _Timer() as t:
        ...
        for merge_with in args.in_[1:]:
            result = _merge.merge_api_structures(
                result, _merge.read_structure_json(merge_with)
            )
        if args.overrides:
            overrides = _merge.read_structure_json(args.overrides)
            result = _merge.apply_overrides(result, overrides)
        _parse.store_structure_json(args.outfile, result, (not args.uncompressed))
