...
def apply_overrides(structure: ApiStructure, overrides: ApiStructure) -> ApiStructure:
    result = {
        "endpoints": override_organized_flows(
            structure.get("endpoints"), overrides.get("endpoints")
        ),
        "entities": override_object_structures(
            structure.get("entities"), overrides.get("entities")
        ),
    }
    return result


def override_organized_flows(
    flows: ApiStructure, overrides: ApiStructure
) -> ApiStructure:
...
def override_object_structures(
    object_structures: _List[_PssObjectStructure], overrides: _List[_PssObjectStructure]
) -> _List[_PssObjectStructure]:
    objects_dict = {
        object_structure.object_type_name: object_structure
        for object_structure in object_structures
    }
    overrides_dict = {
        object_structure.object_type_name: object_structure
        for object_structure in overrides
    }

    object_structure_names = sorted(
        list(set(objects_dict.keys()).union(set(overrides_dict.keys())))
    )
    result = []
...
def convert_entities_dict_to_object_structures(
    ...
    result = {
        _PssObjectStructure(entity_name, properties)
...
def convert_structure_dict_to_organized_flows(
    ...
    for service, endpoints in organized_dict["endpoints"].items():
        for endpoint, flow_dict in endpoints.items():
            result.setdefault(service, {}).setdefault(endpoint, []).append(
                _PssFlowDetails(flow_dict)
            )
    return result
        ...
        if service_1 in flows_2.keys():
            for endpoint_1, flow_details_1 in endpoints_1.items():
                if endpoint_1 in flows_2[service_1]:
                    merged_flows.setdefault(service_1, {}).setdefault(
                        endpoint_1, []
                    ).extend(flow_details_1)
                    merged_flows.setdefault(service_1, {}).setdefault(
                        endpoint_1, []
                    ).extend(flows_2[service_1][endpoint_1])
                else:
        ...
        if service_2 in flows_1.keys():
            for endpoint_2, flow_details_2 in endpoints_2.items():
                if endpoint_2 in flows_1[service_2]:
                    merged_flows.setdefault(service_2, {}).setdefault(
                        endpoint_2, []
                    ).extend(flow_details_2)
                    merged_flows.setdefault(service_2, {}).setdefault(
                        endpoint_2, []
                    ).extend(flows_1[service_2][endpoint_2])
                else:
...
def merge_object_structures(
    ...
    for object_structure_1 in object_structures_1:
        object_structures.setdefault(object_structure_1.object_type_name, []).append(
            object_structure_1
        )
    for object_structure_2 in object_structures_2:
        object_structures.setdefault(object_structure_2.object_type_name, []).append(
            object_structure_2
        )
    ...
    for _, entity_structures in object_structures.items():
        ...
        for merge_with in entity_structures[1:]:
            entity_structure = _parse.merge_object_structures(
                entity_structure, merge_with
            )
...
def merge_api_structures(
    structure_1: ApiStructure, structure_2: ApiStructure, overrides: ApiStructure = None
) -> ApiStructure:
    ...
    result = {
        "endpoints": merge_organized_flows(
            structure_1["endpoints"], structure_2["endpoints"]
        ),
        "entities": merge_object_structures(
            structure_1["entities"], structure_2["entities"]
        ),
    }
    return result
